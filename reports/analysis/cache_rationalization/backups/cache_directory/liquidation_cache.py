...
class LiquidationCache:
    ...
    def save(self, liquidations: List[Dict[str, Any]], symbol: str) -> bool:
        ...
        try:
            # Use consistent naming with append() method
            symbol = symbol.lower()
    ...
    def load(self, symbol: str) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            if not liquidations:
                logger.debug(f"No liquidation data found in cache for {symbol}")
                return None
            
            # Filter expired liquidations (same logic as append method)
            now = time.time() * 1000
            ...
            # Update cache file if any liquidations were expired
            if len(valid_liquidations) != len(liquidations):
                expired_count = len(liquidations) - len(valid_liquidations)
                logger.debug(f"Removing {expired_count} expired liquidations from cache")
                
                # Update the cache file with only valid liquidations
                cache_data['data'] = valid_liquidations
                cache_data['timestamp'] = int(time.time())
                
                with open(cache_file, 'w') as f:
                    json.dump(cache_data, f)
            
            if valid_liquidations:
                cache_age_seconds = time.time() - cache_data.get("timestamp", time.time())
                logger.info(f"Loaded {len(valid_liquidations)} liquidation events from cache (age: {cache_age_seconds / 60:.1f} min)")
                return valid_liquidations
                
            logger.debug(f"All liquidations in cache for {symbol} have expired")
            return None
    ...
    def add_liquidation(self, symbol: str, liquidation: Dict[str, Any]) -> None:
        """
        Add a liquidation event to cache (compatibility method).
        
        Args:
            symbol: Trading pair symbol
            liquidation: Liquidation event data
        """
        self.append(liquidation, symbol)
    
    def get_liquidations(self, symbol: str, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        Get liquidations from cache (compatibility method).
        ...
        """
        liquidations = self.load(symbol) or []
        if limit and len(liquidations) > limit:
            return liquidations[-limit:]  # Return most recent
        return liquidations
    ...
    def append(self, liquidation: Dict[str, Any], symbol: str = None) -> None:
        ...
        try:
            ...
            # Load existing data using structured format
            existing_data = []
            ...
            if os.path.exists(cache_file):
                try:
                    ...
                    # Handle both old (array) and new (structured) formats
                    if isinstance(cache_structure, list):
                        # Old format - just an array
                        existing_data = cache_structure
                        logger.debug(f"Loaded {len(existing_data)} events from old format cache")
                    elif isinstance(cache_structure, dict) and 'data' in cache_structure:
                        # New structured format
                        existing_data = cache_structure.get('data', [])
                        cache_timestamp = cache_structure.get('timestamp', time.time())
                        logger.debug(f"Loaded {len(existing_data)} events from structured cache")
                    else:
                        logger.warning(f"Unrecognized cache format: {cache_file}")
                        existing_data = []
            ...
            # Save updated cache in structured format
            cache_structure = {
