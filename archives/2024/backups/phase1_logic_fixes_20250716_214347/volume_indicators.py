...
class VolumeIndicators(BaseIndicator, DebugLoggingMixin):
    ...
    def _calculate_volume_trend_score(self, df: pd.DataFrame) -> float:
        ...
        try:
            ...
            trend_score = talib.SMA(vol_change, timeperiod=10) * 100
    ...
    def _calculate_base_volume_score(self, df: pd.DataFrame) -> float:
        ...
        try:
            ...
            volume_ma = pd.Series(talib.SMA(volume.values.astype(float), timeperiod=20), index=volume.index)
            volume_std = pd.Series(talib.STDDEV(volume.values.astype(float), timeperiod=20), index=volume.index)
