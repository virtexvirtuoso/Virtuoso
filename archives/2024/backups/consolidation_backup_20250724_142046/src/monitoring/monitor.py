...
class MarketMonitor:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if metrics_manager:
            ...
            # Create a minimal metrics manager if none provided
        ...
        # Note: Actual initialization is deferred to start() method since __init__ is not async
    ...
    async def _initialize_websocket(self) -> None:
        ...
        try:
            ...
            # If specific symbol is provided, use it
            if self.symbol_str:
                symbols = [self.symbol_str]
                self.logger.info(f"Initializing WebSocket for single symbol: {self.symbol_str}")
            # Otherwise, check if we have TopSymbolsManager
            elif self.top_symbols_manager:
                # Get top symbols from the manager
                top_symbols = await self.top_symbols_manager.get_top_symbols()
                if top_symbols:
                    # Extract symbol names from the top symbols data
                    symbols = [s['symbol'].replace('/', '') for s in top_symbols[:10]]  # Limit to top 10
                    self.logger.info(f"Initializing WebSocket for {len(symbols)} top symbols: {symbols}")
                else:
                    self.logger.warning("TopSymbolsManager returned no symbols, WebSocket initialization skipped")
                    return
            else:
                self.logger.info("No symbol or TopSymbolsManager provided, WebSocket initialization skipped")
                return
            ...
            asyncio.get_event_loop().create_task(self.ws_manager.initialize(symbols))
            
            self.logger.info(f"WebSocket integration initialized for {len(symbols)} symbol(s)")
    ...
    async def _process_orderbook_update(self, message: Dict[str, Any]) -> None:
        ...
        """
        try:
            # Performance optimization: Throttle orderbook updates
            current_time = time.time()
            if hasattr(self, '_last_orderbook_update'):
                if current_time - self._last_orderbook_update < 0.1:  # Max 10 updates per second
                    return
            self._last_orderbook_update = current_time
            ...
            # Performance optimization: Limit orderbook depth to what's actually needed
            ...
            orderbook = {
                ...
                'bids': orderbook_data.get('bids', [])[:MAX_ORDERBOOK_LEVELS],  # Limit depth
                'asks': orderbook_data.get('asks', [])[:MAX_ORDERBOOK_LEVELS]   # Limit depth
            }
            
            # Performance optimization: Process orderbook asynchronously if sorting is needed
            # Most exchanges send pre-sorted data, so we only sort if needed
            needs_bid_sort = False
            needs_ask_sort = False
            
            if orderbook['bids'] and len(orderbook['bids']) > 1:
                # Check if already sorted
                if float(orderbook['bids'][0][0]) < float(orderbook['bids'][1][0]):
                    needs_bid_sort = True
                    
            if orderbook['asks'] and len(orderbook['asks']) > 1:
                # Check if already sorted
                if float(orderbook['asks'][0][0]) > float(orderbook['asks'][1][0]):
                    needs_ask_sort = True
            
            # Sort asynchronously if needed
            if needs_bid_sort or needs_ask_sort:
                loop = asyncio.get_event_loop()
                
                async def sort_orderbook():
                    if needs_bid_sort:
                        orderbook['bids'] = await loop.run_in_executor(
                            None, 
                            lambda: sorted(orderbook['bids'], key=lambda x: float(x[0]), reverse=True)
                        )
                    if needs_ask_sort:
                        orderbook['asks'] = await loop.run_in_executor(
                            None,
                            lambda: sorted(orderbook['asks'], key=lambda x: float(x[0]))
                        )
                
                await sort_orderbook()
            ...
            if not hasattr(self, '_last_orderbook_log') or current_time - self._last_orderbook_log > 5:
                self.logger.debug(f"Updated orderbook from WebSocket: {len(orderbook['bids'])} bids, {len(orderbook['asks'])} asks")
                self._last_orderbook_log = current_time
                ...
                if hasattr(self, 'liquidation_detector') and self.liquidation_detector:
                    try:
                        ...
                        liquidation_event = LiquidationEvent(
                            ...
                            market_depth_impact=0.0,  # Would need market depth data
                            volatility_spike=1.0,  # Would need historical volatility data
                            duration_seconds=0,  # Instantaneous for real-time events
    ...
    async def is_healthy(self) -> bool:
        """Check if the market monitor is healthy.
        
        Returns:
            bool: True if the market monitor is healthy, False otherwise
        """
        try:
            ...
            if self.exchange_manager:
                try:
                    exchange_healthy = await self.exchange_manager.is_healthy()
                    if not exchange_healthy:
                        return False
                except Exception as e:
                    self.logger.error(f"Failed to check exchange health: {str(e)}", exc_info=True)
                    return False
            
            # Check if market data manager is healthy
            if self.market_data_manager:
                try:
                    # Check if market data manager has recent data
                    stats = self.market_data_manager.get_stats()
                    if not stats:
                        return False
                    
                    # Check if websocket is connected (if enabled)
                    websocket_status = stats.get('websocket', {})
                    if websocket_status.get('enabled', False):
                    ...
                    self.logger.error(f"Failed to check market data manager health: {str(e)}", exc_info=True)
                    return False
            ...
            self.logger.error(f"Error checking market monitor health: {str(e)}")
            return False
    ...
    async def _process_analysis_result(self, symbol: str, result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Enhanced debugging: Log confluence analysis process
            ...
            # Log component scores with context
            self.logger.debug("=== Component Scores ===")
            for component, score in components.items():
                ...
                self.logger.debug(f"{component}: {score:.2f} ({bias} bias)")
            ...
            if not result.get('market_interpretations') and hasattr(self, 'signal_generator') and self.signal_generator:
                try:
                    ...
                    self.logger.debug("This is NORMAL OPERATION - interpretations are generated dynamically")
                ...
                self.logger.debug(f"Using pre-existing interpretations for {symbol}")
            
            self.logger.debug(f"=== CONFLUENCE ANALYSIS PROCESS DEBUG COMPLETE ===")
