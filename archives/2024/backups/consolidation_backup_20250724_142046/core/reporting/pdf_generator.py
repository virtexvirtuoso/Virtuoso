...
class ReportGenerator:
    ...
    def _create_candlestick_chart(
        ...
        try:
            ...
            # Validate and convert OHLC columns to float64 for mplfinance
            ohlc_columns = ["open", "high", "low", "close", "volume"]
            for col in ohlc_columns:
                if col in df.columns:
                    # Convert to numeric, coercing errors to NaN
                    df[col] = pd.to_numeric(df[col], errors='coerce')
                    # Fill any NaN values with forward fill then 0
                    df[col] = df[col].ffill().fillna(0)
                    # Ensure float64 dtype
                    df[col] = df[col].astype(np.float64)
                    
            # Validate datetime index is monotonic
            if not df.index.is_monotonic_increasing:
                self._log("Sorting DataFrame by index to ensure monotonic datetime", logging.DEBUG)
                df = df.sort_index()
    ...
    def _prepare_for_json(self, obj: Any, visited: Optional[set] = None) -> Any:
        ...
        try:
            if isinstance(obj, dict):
                ...
                try:
                    return self._prepare_for_json(obj.__dict__, visited)
                except Exception:
    ...
    def generate_trading_report(
        ...
        try:
            ...
            stop_loss_percent = 0
            try:
                # Get trade parameters if available (consistent with chart generation)
                trade_params = signal_data.get("trade_params", {})
