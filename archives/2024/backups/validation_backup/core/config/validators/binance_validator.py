...
This module provides comprehensive validation for Binance-specific configuration
settings to ensure proper integration with the Virtuoso trading system.
"""
...
class BinanceConfigValidator:
    ...
    def __init__(self):
        ...
        self.valid_quote_currencies = [
            'USDT', 'BTC', 'ETH', 'BNB', 'BUSD', 'USDC', 'DAI', 'TUSD'
        ]
        
        # Rate limit boundaries
        self.rate_limit_bounds = {
            'requests_per_minute': {'min': 1, 'max': 1200},
            'requests_per_second': {'min': 1, 'max': 20},
            'weight_per_minute': {'min': 1, 'max': 6000}
        }
        ...
        try:
            ...
            for section in self.required_sections:
                if section not in config:
                    errors.append(BinanceValidationError(
                        field=section,
    ...
    def validate_api_credentials(self, credentials: Dict[str, Any]) -> ValidationResult:
        ...
        if not api_key and not api_secret:
            warnings.append(BinanceValidationError(
                field="api_credentials",
                message="No API credentials provided - will use public access only",
                severity="warning"
            ))
        elif api_key and not api_secret:
            errors.append(BinanceValidationError(
                field="api_secret",
                message="API secret is required when API key is provided",
                severity="error"
            ))
        elif not api_key and api_secret:
            errors.append(BinanceValidationError(
                field="api_key",
                message="API key is required when API secret is provided",
                severity="error"
            ))
        else:
            ...
            if api_key and len(api_key) not in [0, 64]:
                warnings.append(BinanceValidationError(
                    field="api_key",
                    message="API key length is unusual (expected 64 characters for Binance)",
                    severity="warning"
                ))
            
            # Validate API secret format (Binance API secrets are typically 64 characters)
            if api_secret and len(api_secret) not in [0, 64]:
                warnings.append(BinanceValidationError(
                    field="api_secret",
                    message="API secret length is unusual (expected 64 characters for Binance)",
                    severity="warning"
                ))
    ...
    def validate_rate_limits(self, rate_limits: Dict[str, Any]) -> ValidationResult:
        ...
        for field in required_fields:
            if field not in rate_limits:
                errors.append(BinanceValidationError(
                    field=field,
                    message=f"Required rate limit field '{field}' is missing",
                    severity="error"
                ))
        ...
        for field, bounds in self.rate_limit_bounds.items():
            if field in rate_limits:
                ...
                if not isinstance(value, (int, float)) or value <= 0:
                    ...
                    warnings.append(BinanceValidationError(
                        field=field,
                        message=f"{field} value {value} is outside recommended range {bounds['min']}-{bounds['max']}",
                        severity="warning"
                    ))
        
        # Check for reasonable rate limiting
        ...
        if rpm > 0 and rps > 0:
            ...
            if rps > max_rps_from_rpm:
                warnings.append(BinanceValidationError(
                    field="rate_limits",
                    message=f"requests_per_second ({rps}) exceeds what requests_per_minute allows ({max_rps_from_rpm:.2f})",
                    severity="warning"
                ))
    ...
    def validate_websocket_config(self, websocket: Dict[str, Any]) -> ValidationResult:
        ...
        # Check required WebSocket fields
        required_fields = ['public']
        for field in required_fields:
            if field not in websocket:
                errors.append(BinanceValidationError(
                    field=f"websocket.{field}",
                    message=f"Required WebSocket field '{field}' is missing",
                    severity="error"
                ))
        ...
        for field in url_fields:
            if field in websocket:
                ...
                if not self._is_valid_websocket_url(url):
                    errors.append(BinanceValidationError(
                        field=f"websocket.{field}",
                        message=f"Invalid WebSocket URL format: {url}",
                        severity="error"
                    ))
        
        # Validate numeric settings
        numeric_fields = {
            'ping_interval': {'min': 1, 'max': 300},
            'reconnect_attempts': {'min': 1, 'max': 10}
        }
        ...
        for field, bounds in numeric_fields.items():
            if field in websocket:
                ...
                if not isinstance(value, (int, float)) or value < bounds['min'] or value > bounds['max']:
                    warnings.append(BinanceValidationError(
                        field=f"websocket.{field}",
                        message=f"{field} should be between {bounds['min']} and {bounds['max']}",
                        severity="warning"
                    ))
    ...
    def validate_market_types(self, market_types: List[str]) -> ValidationResult:
        ...
        if not isinstance(market_types, list):
            errors.append(BinanceValidationError(
                field="market_types",
                message="market_types must be a list",
                severity="error"
            ))
            return ValidationResult(is_valid=False, errors=errors, warnings=warnings)
        ...
        if not market_types:
            warnings.append(BinanceValidationError(
                field="market_types",
                message="No market types specified - defaulting to spot",
                severity="warning"
            ))
        
        # Validate each market type
        for market_type in market_types:
            if market_type not in self.valid_market_types:
                errors.append(BinanceValidationError(
                    field="market_types",
                    message=f"Invalid market type '{market_type}'. Valid types: {self.valid_market_types}",
                    severity="error"
                ))
    ...
    def validate_data_preferences(self, preferences: Dict[str, Any]) -> ValidationResult:
        ...
        if 'preferred_quote_currencies' in preferences:
            ...
            if not isinstance(quote_currencies, list):
                errors.append(BinanceValidationError(
                    field="data_preferences.preferred_quote_currencies",
                    message="preferred_quote_currencies must be a list",
                    severity="error"
                ))
            else:
                for currency in quote_currencies:
                    if currency not in self.valid_quote_currencies:
                        warnings.append(BinanceValidationError(
                            field="data_preferences.preferred_quote_currencies",
                            message=f"Unusual quote currency '{currency}'. Common currencies: {self.valid_quote_currencies[:5]}",
                            severity="warning"
                        ))
        ...
        if 'min_24h_volume' in preferences:
            ...
            if not isinstance(volume, (int, float)) or volume < 0:
                errors.append(BinanceValidationError(
                    field="data_preferences.min_24h_volume",
                    message="min_24h_volume must be a non-negative number",
                    severity="error"
                ))
            elif volume < 100000:
                warnings.append(BinanceValidationError(
                    field="data_preferences.min_24h_volume",
                    message="Very low volume threshold may include illiquid markets",
                    severity="warning"
                ))
    ...
    def validate_top_level_settings(self, config: Dict[str, Any]) -> ValidationResult:
            ...
            if field in config and not isinstance(config[field], bool):
                ...
                if isinstance(config[field], str):
                    if config[field].lower() in ['true', 'false']:
                        warnings.append(BinanceValidationError(
                            field=field,
                            message=f"{field} should be boolean, not string '{config[field]}'",
                            severity="warning"
                        ))
                    else:
                        errors.append(BinanceValidationError(
                            field=field,
                            message=f"{field} must be boolean (true/false)",
                            severity="error"
                        ))
                else:
                    errors.append(BinanceValidationError(
        ...
        if config.get('primary', False) and config.get('data_only', True):
            warnings.append(BinanceValidationError(
                field="configuration",
                message="Primary exchange is set to data_only mode - this may limit functionality",
                severity="warning"
            ))
