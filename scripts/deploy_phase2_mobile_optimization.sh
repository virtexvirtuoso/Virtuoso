...
PERF_RESPONSE=$(curl -s "http://VPS_HOST_REDACTED:8003/api/dashboard/mobile-performance")
...
for i in {1..3}; do
    RESPONSE_TIME=$(curl -w "%{time_total}" -s -o /dev/null "http://VPS_HOST_REDACTED:8003/api/dashboard/mobile-data")
...
if [ "$CONFLUENCE_COUNT" -gt "0" ]; then
    ...
    echo "   Monitor with: curl http://VPS_HOST_REDACTED:8003/api/dashboard/mobile-performance"
fi

echo ""
echo "📊 Monitor Phase 2 performance:"
echo "   curl http://VPS_HOST_REDACTED:8003/api/dashboard/mobile-performance"
echo ""
echo "🔍 View logs:"
