...
ssh linuxuser@5.223.63.4 'cd /home/linuxuser/trading/Virtuoso_ccxt && pgrep -f "python.*web_server" > /dev/null || (source venv311/bin/activate && nohup python src/web_server.py > web_server.log 2>&1 & echo "Started new server process")'
...
echo "   Dashboard: $(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://5.223.63.4:8003/dashboard)"
echo "   Mobile: $(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://5.223.63.4:8003/dashboard/mobile)"
echo "   API Overview: $(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://5.223.63.4:8003/api/dashboard/overview)"
...
start_time=$(date +%s.%N)
response=$(curl -s --max-time 15 http://5.223.63.4:8003/api/market/overview)
end_time=$(date +%s.%N)
...
start_time=$(date +%s.%N)
response=$(curl -s --max-time 5 http://5.223.63.4:8003/api/market/overview)
end_time=$(date +%s.%N)
duration=$(echo "$end_time - $start_time" | bc)
...
response=$(curl -s --max-time 5 http://5.223.63.4:8003/api/market/overview 2>&1)
