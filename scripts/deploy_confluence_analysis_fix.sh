...
ssh linuxuser@${VPS_HOST} "cd /home/linuxuser/trading/Virtuoso_ccxt && sudo systemctl restart virtuoso-web"
...
ssh linuxuser@${VPS_HOST} "sudo systemctl status virtuoso-web --no-pager -l"
...
curl -s http://${VPS_HOST}:8001/api/confluence/latest | jq -C '.' || echo "❌ Latest confluence endpoint not responding"

echo ""
echo "Testing /api/dashboard/confluence-analysis/BTCUSDT:"
curl -s "http://${VPS_HOST}:8001/api/dashboard/confluence-analysis/BTCUSDT" | jq -C '.' || echo "❌ Analysis endpoint not responding"

echo ""
echo "Testing /api/dashboard/confluence-analysis-page:"
curl -s -I "http://${VPS_HOST}:8001/api/dashboard/confluence-analysis-page?symbol=BTCUSDT" | head -1

echo ""
echo "🎯 Confluence Analysis Fix Deployment Complete!"
echo "✅ Analysis button should now show full terminal breakdown"
echo "📱 Test on mobile dashboard: http://${VPS_HOST}:8001/dashboard/mobile"