#!/usr/bin/env python3
"""
Bitcoin Beta Data Collection Service with Dynamic Symbol Selection
...
from scripts.bitcoin_beta_symbol_selector import BetaSymbolSelector
...
class BitcoinBetaDataServiceDynamic:
    """Service to collect and cache historical price data with dynamic symbol selection"""
    
    def __init__(self):
        self.cache = Client('localhost', 11211)
        self.base_url = 'https://api.bybit.com'
        self.symbol_selector = BetaSymbolSelector()
        
        # Dynamic symbols (will be updated daily)
        self.symbols = []
        
        # Intervals to fetch
        self.intervals = {
            '60': 2160,    # 1h: 90 days of data (reduced from 2160 for faster initial load)
        ...
        self.symbol_update_interval = 86400  # Update symbol list daily
        self.last_symbol_update = 0
        
    async def __aenter__(self):
        """Async context manager entry"""
        self.session = aiohttp.ClientSession()
        await self.symbol_selector.__aenter__()
        return self
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        await self.symbol_selector.__aexit__(exc_type, exc_val, exc_tb)
    ...
    async def update_symbols(self):
        ...
        if current_time - self.last_symbol_update > self.symbol_update_interval:
            ...
            self.symbols = await self.symbol_selector.get_current_symbols()
            self.last_symbol_update = current_time
            logger.info(f"Symbol list updated: {len(self.symbols)} symbols")
            
            # Log top 5 symbols
            if len(self.symbols) > 5:
                logger.info(f"Top 5: {', '.join(self.symbols[:5])}")
    ...
    async def fetch_all_symbols(self):
        ...
        for i in range(0, len(self.symbols), batch_size):
            ...
            tasks = [self.fetch_and_store_symbol(symbol) for symbol in batch]
            await asyncio.gather(*tasks)
            
            logger.info(f"Completed batch {i//batch_size + 1}/{(len(self.symbols) + batch_size - 1)//batch_size}")
    ...
    async def update_recent_data(self):
        ...
        logger.info(f"Updating recent data for {len(self.symbols)} symbols")
    ...
    async def get_data_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            'symbol_list_age': (time.time() - self.last_symbol_update) / 3600  # hours
        }
        
        for symbol in self.symbols[:5]:  # Check first 5 for status
            symbol_status = {}
    ...
    async def run_continuous(self):
        ...
        while True:
            try:
                ...
                # Check if symbol list needs updating (daily)
                if current_time - self.last_symbol_update > self.symbol_update_interval:
                    old_symbols = self.symbols.copy()
                    await self.update_symbols()
                    
                    # If symbols changed, fetch full data for new symbols
                    new_symbols = set(self.symbols) - set(old_symbols)
                    if new_symbols:
                        logger.info(f"Fetching data for {len(new_symbols)} new symbols")
                ...
                if current_time - last_full_update > 21600:
                ...
                logger.info(f"Data status: {len(self.symbols)} symbols tracked, "
                          f"symbol list age: {status['symbol_list_age']:.1f}h")
...
async def main():
    """Main entry point"""
    async with BitcoinBetaDataServiceDynamic() as service:
        ...
        if len(sys.argv) > 1 and sys.argv[1] == '--status':
            ...
            await service.update_symbols()
            print(f"Selected {len(service.symbols)} symbols:")
            for i, symbol in enumerate(service.symbols[:10], 1):
                print(f"{i:2}. {symbol}")
        else:
            # Run continuously
