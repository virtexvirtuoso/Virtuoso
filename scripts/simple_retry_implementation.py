...
def implement_simple_retry():
    ...
    # Create backup
    backup_cmd = 'ssh linuxuser@45.77.40.77 "cd /home/linuxuser/trading/Virtuoso_ccxt && cp src/core/exchanges/bybit.py src/core/exchanges/bybit.py.backup_retry_$(date +%Y%m%d_%H%M%S)"'
...
'''
    ...
    # Copy to VPS
    copy_cmd = 'scp /tmp/retry_method.py linuxuser@45.77.40.77:/tmp/'
    ...
    insert_cmd = '''ssh linuxuser@45.77.40.77 "cd /home/linuxuser/trading/Virtuoso_ccxt && awk '/async def _make_request\\(/ {p=1} p && /^[[:space:]]*async def|^[[:space:]]*def/ && !/async def _make_request/ {print \\"\\n\\" ; system(\\"cat /tmp/retry_method.py\\"); p=0} 1' src/core/exchanges/bybit.py > /tmp/bybit_with_retry.py && mv /tmp/bybit_with_retry.py src/core/exchanges/bybit.py"'''
    ...
    if result.returncode != 0:
        ...
        find_line_cmd = 'ssh linuxuser@45.77.40.77 "grep -n \'async def _process_response\' /home/linuxuser/trading/Virtuoso_ccxt/src/core/exchanges/bybit.py | head -1 | cut -d: -f1"'
        ...
        if line_result.returncode == 0 and line_result.stdout.strip():
            ...
            insert_alt_cmd = f'''ssh linuxuser@45.77.40.77 "cd /home/linuxuser/trading/Virtuoso_ccxt && sed -i '{line_num-1} r /tmp/retry_method.py' src/core/exchanges/bybit.py"'''
    ...
    for method in methods:
        ...
        update_cmd = f'''ssh linuxuser@45.77.40.77 "cd /home/linuxuser/trading/Virtuoso_ccxt && sed -i '/{method}/,/async def\\|def/ s/await self._make_request(/await self._make_request_with_retry(/g' src/core/exchanges/bybit.py"'''
    ...
    # Restart service
    restart_cmd = 'ssh linuxuser@45.77.40.77 "sudo systemctl restart virtuoso.service"'
    print("\n🔄 Restarting service...")
    ...
    # Check status
    status_cmd = 'ssh linuxuser@45.77.40.77 "sudo systemctl is-active virtuoso.service"'
    ...
    if "active" in result.stdout:
        ...
        # Get new PID
        pid_cmd = 'ssh linuxuser@45.77.40.77 "ps aux | grep \'python.*main.py\' | grep -v grep | awk \'{print $2}\'"'
        ...
        # Check error logs
        error_cmd = 'ssh linuxuser@45.77.40.77 "sudo journalctl -u virtuoso.service --since \'1 minute ago\' | grep -E \'(ERROR|error|SyntaxError)\' | head -5"'
...
def monitor_retry_activity(pid, duration_minutes=3):
    ...
    while time.time() - start_time < duration_minutes * 60:
        ...
        retry_cmd = 'ssh linuxuser@45.77.40.77 "sudo journalctl -u virtuoso.service --since \'30 seconds ago\' | grep -i \'retry\\|retrying\'"'
        retry_result = subprocess.run(retry_cmd, shell=True, capture_output=True, text=True)
        
        # Check for timeouts
        timeout_cmd = 'ssh linuxuser@45.77.40.77 "sudo journalctl -u virtuoso.service --since \'30 seconds ago\' | grep -c \'timeout\'"'
    ...
    # Final check
    final_cmd = 'ssh linuxuser@45.77.40.77 "sudo journalctl -u virtuoso.service --since \'3 minutes ago\' | grep -c -i retry"'
