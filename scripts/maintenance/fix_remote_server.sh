...
echo "🔧 Fixing Virtuoso server issues on ${VPS_HOST}..."
...
test_route() {
    ...
    local desc=$2
    echo -n "Testing $desc... "
    status=$(curl -s -o /dev/null -w "%{http_code}" "http://${VPS_HOST}:8003$path")
    if [ "$status" = "200" ]; then
        echo "✅ OK ($status)"
    else
        echo "❌ Failed ($status)"
...
time_taken=$(curl -s -o /dev/null -w "%{time_total}" "http://${VPS_HOST}:8003/api/market/overview")
echo "${time_taken}s"
...
echo "  Legacy:  http://${VPS_HOST}:8003/dashboard/v10"