...
# Test the API
start_time=$(date +%s.%N)
response=$(curl -s -w "\n%{http_code}" --max-time 10 "http://VPS_HOST_REDACTED:8003/api/market/overview")
...
if [ "$status_code" = "200" ]; then
    ...
    start_time=$(date +%s.%N)
    curl -s "http://VPS_HOST_REDACTED:8003/api/market/overview" | python3 -m json.tool | grep -E "(cached|fetch_time)"
    end_time=$(date +%s.%N)
