...
sleep 2
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://5.223.63.4:8003/api/system/health 2>/dev/null || echo "000")
if [ "$HTTP_CODE" = "200" ]; then
    ...
    curl -s http://5.223.63.4:8003/api/system/health | python3 -m json.tool 2>/dev/null | head -20 || echo "   Could not parse health response"
else
...
echo ""
echo "Test endpoints:"
echo "  curl http://5.223.63.4:8003/api/system/health"
echo "  curl http://5.223.63.4:8001/"