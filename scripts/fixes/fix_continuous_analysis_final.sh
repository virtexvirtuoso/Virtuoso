...
# Create backup
ssh linuxuser@${VPS_HOST} "cp /home/linuxuser/trading/Virtuoso_ccxt/src/main.py /home/linuxuser/trading/Virtuoso_ccxt/src/main.py.backup_final"
...
scp /tmp/fix_main.py linuxuser@${VPS_HOST}:/tmp/
ssh linuxuser@${VPS_HOST} "python3 /tmp/fix_main.py"
...
if ssh linuxuser@${VPS_HOST} "cd /home/linuxuser/trading/Virtuoso_ccxt && /home/linuxuser/trading/Virtuoso_ccxt/venv311/bin/python -m py_compile src/main.py 2>&1"; then
    ...
    if ssh linuxuser@${VPS_HOST} "sudo journalctl -u virtuoso.service --since '1 minute ago' | grep -q 'Continuous analysis manager started'"; then
        ...
        ssh linuxuser@${VPS_HOST} "sudo journalctl -u virtuoso.service --since '1 minute ago' | grep 'ContinuousAnalysisManager' | tail -3"
    fi
    
    # Show cache status
    echo -e "\n📝 Testing cache endpoints:"
    echo "Analysis results:"
    ssh linuxuser@${VPS_HOST} "curl -s http://localhost:8000/api/analysis/results | head -c 100"
    echo -e "\n"
else
    echo "❌ Syntax validation failed - restoring backup"
    ssh linuxuser@${VPS_HOST} "mv /home/linuxuser/trading/Virtuoso_ccxt/src/main.py.backup_final /home/linuxuser/trading/Virtuoso_ccxt/src/main.py"
fi
