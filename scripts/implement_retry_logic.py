...
def apply_retry_logic_to_vps():
    ...
    copy_cmd = f'scp {patch_file} linuxuser@VPS_HOST_REDACTED:/tmp/patch_retry_logic.py'
    ...
    # Apply the patch
    apply_cmd = 'ssh linuxuser@VPS_HOST_REDACTED "cd /home/linuxuser/trading/Virtuoso_ccxt && ./venv311/bin/python /tmp/patch_retry_logic.py src/core/exchanges/bybit.py"'
...
def monitor_retry_effectiveness(pid, duration_minutes=5):
    ...
    # Initial check
    initial_cmd = 'ssh linuxuser@VPS_HOST_REDACTED "sudo journalctl -u virtuoso.service --since \'1 minute ago\' | grep -E \'(retry|Retry|Connection timeout|Request timeout)\'"'
    ...
    while time.time() - start_time < duration_minutes * 60:
        ...
        # Count retries
        retry_cmd = f'ssh linuxuser@VPS_HOST_REDACTED "sudo journalctl -u virtuoso.service --since \'{elapsed} minutes ago\' | grep -c -i retry"'
        ...
        # Count timeouts
        timeout_cmd = f'ssh linuxuser@VPS_HOST_REDACTED "sudo journalctl -u virtuoso.service --since \'{elapsed} minutes ago\' | grep -c \'timeout\'"'
