...
def monitor_timeout_improvements(duration_minutes=5):
    ...
    cmd_timeout = 'ssh linuxuser@45.77.40.77 "sudo journalctl -u virtuoso.service --since \'5 minutes ago\' | grep -c \'Request timeout\'"'
    result = subprocess.run(cmd_timeout, shell=True, capture_output=True, text=True)
    baseline_timeouts = int(result.stdout.strip() or 0)
    
    cmd_empty = 'ssh linuxuser@45.77.40.77 "sudo journalctl -u virtuoso.service --since \'5 minutes ago\' | grep -c \'Empty DataFrame\'"'
    ...
    while time.time() - start_time < duration_minutes * 60:
        ...
        cmd_recent_timeout = f'ssh linuxuser@45.77.40.77 "sudo journalctl -u virtuoso.service --since \'{elapsed_minutes + 1} minutes ago\' | grep \'Request timeout after 10s\'"'
        ...
        cmd_recent_empty = f'ssh linuxuser@45.77.40.77 "sudo journalctl -u virtuoso.service --since \'{elapsed_minutes + 1} minutes ago\' | grep \'Empty DataFrame\'"'
    ...
    # Get final counts
    cmd_final_timeout = f'ssh linuxuser@45.77.40.77 "sudo journalctl -u virtuoso.service --since \'{duration_minutes} minutes ago\' | grep -c \'Request timeout after 10s\'"'
    ...
    cmd_final_empty = f'ssh linuxuser@45.77.40.77 "sudo journalctl -u virtuoso.service --since \'{duration_minutes} minutes ago\' | grep -c \'Empty DataFrame\'"'
    ...
    cmd_status = 'ssh linuxuser@45.77.40.77 "sudo systemctl is-active virtuoso.service"'
