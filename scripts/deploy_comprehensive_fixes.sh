...
scp src/monitoring/components/alerts/alert_manager_refactored.py linuxuser@45.77.40.77:/home/linuxuser/trading/Virtuoso_ccxt/src/monitoring/components/alerts/

# Copy the dependency injection fixes
echo "  → Copying src/core/di/registration.py"
scp src/core/di/registration.py linuxuser@45.77.40.77:/home/linuxuser/trading/Virtuoso_ccxt/src/core/di/

echo "  → Copying src/monitoring/monitor_refactored.py"
scp src/monitoring/monitor_refactored.py linuxuser@45.77.40.77:/home/linuxuser/trading/Virtuoso_ccxt/src/monitoring/

echo "  → Copying src/main.py"
scp src/main.py linuxuser@45.77.40.77:/home/linuxuser/trading/Virtuoso_ccxt/src/
...
if ssh linuxuser@45.77.40.77 "sudo systemctl is-active virtuoso.service" | grep -q "active"; then
    echo "✅ Service is running"
else
    echo "❌ Service failed to start"
    echo "📋 Service logs:"
    ssh linuxuser@45.77.40.77 "sudo journalctl -u virtuoso.service --lines=30 --no-pager"
    exit 1
fi
...
echo "   ssh linuxuser@45.77.40.77 'sudo journalctl -u virtuoso.service -f'"
echo ""
echo "🧪 Test the system with:"
echo "   curl http://45.77.40.77:8003/api/dashboard/data"
echo "   curl http://45.77.40.77:8003/health"
echo ""
echo "🔍 Check for error resolution:"
echo "   ssh linuxuser@45.77.40.77 'sudo journalctl -u virtuoso.service --since=\"1 minute ago\" | grep -E \"(ERROR|WARNING|✅|SUCCESS)\"'"