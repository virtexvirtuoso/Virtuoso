...
scp src/api/routes/dashboard_cached.py linuxuser@5.223.63.4:/home/linuxuser/trading/Virtuoso_ccxt/src/api/routes/
...
ssh linuxuser@5.223.63.4 'pkill -f "web_server.py" && sleep 2 && cd /home/linuxuser/trading/Virtuoso_ccxt && PYTHONPATH=/home/linuxuser/trading/Virtuoso_ccxt nohup venv311/bin/python src/web_server.py > logs/web_mobile_fix.log 2>&1 & echo "Web server restarted with PID: $!"'
...
curl -s http://5.223.63.4:8001/api/dashboard/mobile-data | python3 -c "
import sys, json
data = json.load(sys.stdin)
print(f'  Status: {data.get(\"status\")}')
print(f'  Confluence Scores: {len(data.get(\"confluence_scores\", []))} items')
