...
curl -s http://5.223.63.4:8001/api/cache/health | python3 -m json.tool | head -n 10

echo ""
echo "2. Testing Phase 2 dashboard:"
curl -s http://5.223.63.4:8001/dashboard/phase2 | grep -o "<title>.*</title>" | head -n 1

echo ""
echo "3. Testing cache overview (Phase 2):"
curl -s http://5.223.63.4:8001/api/cache/cache/overview | python3 -m json.tool 2>/dev/null | head -n 10
...
for endpoint in overview market-overview signals; do
    ...
    response=$(curl -s -w "\n[HTTP: %{http_code}, Time: %{time_total}s]" http://5.223.63.4:8001/api/dashboard-cached/${endpoint})
    echo "     ${response}" | tail -n 1
done

echo ""
echo "5. Testing regular dashboard endpoints (fallback):"
curl -s http://5.223.63.4:8001/api/dashboard/overview | python3 -m json.tool 2>/dev/null | head -n 5
