...
curl -s "http://${VPS_HOST}:8001/dashboard/mobile" | grep -q "MARKET REGIME" && echo "   ✅ Found original MARKET REGIME label" || echo "   ❌ Original structure not found"

echo ""
echo "2. Checking page loads correctly:"
TITLE=$(curl -s "http://${VPS_HOST}:8001/dashboard/mobile" | grep -o '<title>.*</title>')
...
echo "   URL: http://${VPS_HOST}:8001/dashboard/mobile"
echo ""
