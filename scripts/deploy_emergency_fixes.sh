...
scp src/api/routes/dashboard_optimized.py linuxuser@45.77.40.77:/home/linuxuser/trading/Virtuoso_ccxt/src/api/routes/
...
for endpoint in "mobile-data" "overview" "alerts" "opportunities" "health"; do
    ...
    response=$(curl -s -w "\nHTTP %{http_code} - %{time_total}s" -o /tmp/response.json http://45.77.40.77:8003/api/dashboard-cached/$endpoint 2>/dev/null | tail -1)
    echo "$response"
done

echo -e "\n\033[0;32m🎉 Emergency deployment complete!\033[0m"
echo "Monitor with: ssh linuxuser@45.77.40.77 'sudo journalctl -u virtuoso.service -f'"
