...
scp src/api/routes/dashboard_optimized.py linuxuser@${VPS_HOST}:/home/linuxuser/trading/Virtuoso_ccxt/src/api/routes/
...
for endpoint in "mobile-data" "overview" "alerts" "opportunities" "health"; do
    ...
    response=$(curl -s -w "\nHTTP %{http_code} - %{time_total}s" -o /tmp/response.json http://${VPS_HOST}:8003/api/dashboard-cached/$endpoint 2>/dev/null | tail -1)
    echo "$response"
done

echo -e "\n\033[0;32m🎉 Emergency deployment complete!\033[0m"
echo "Monitor with: ssh linuxuser@${VPS_HOST} 'sudo journalctl -u virtuoso.service -f'"
