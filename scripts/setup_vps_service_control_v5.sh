...
display_summary() {
    ...
    if curl -s -o /dev/null -w "%{http_code}" http://localhost:8001/dashboard 2>/dev/null | grep -q "200"; then
        echo -e "Dashboard:       ${GREEN}● ACCESSIBLE${NC} (http://${VPS_HOST}:8001/dashboard)"
    else
...
case "$1" in
    ...
    help|--help|-h)
        ...
        echo -e "${GREEN}http://${VPS_HOST}:8001/dashboard${NC} - Main trading dashboard"
        echo ""
        ...
        echo -e "${BOLD}──────────────────${NC}"
        echo "  Dashboard:      http://${VPS_HOST}:8001/dashboard"
        echo "  Mobile:         http://${VPS_HOST}:8001/mobile"
        echo "  API Health:     http://${VPS_HOST}:8001/api/health"
        echo "  Market Data:    http://${VPS_HOST}:8001/api/market/overview"
        echo "  Trading Signals: http://${VPS_HOST}:8001/api/signals"
        echo ""
    ...
    *)
        ...
        echo -e "${GREEN}🌐 Dashboard: http://${VPS_HOST}:8001/dashboard${NC}"
        exit 1
        ;;
esac
