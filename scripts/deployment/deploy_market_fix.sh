...
scp src/api/routes/market.py linuxuser@VPS_HOST_REDACTED:/home/linuxuser/trading/Virtuoso_ccxt/src/api/routes/
...
time_start=$(date +%s.%N)
response=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 "http://VPS_HOST_REDACTED:8003/api/market/overview")
time_end=$(date +%s.%N)
...
if [ "$response" = "200" ]; then
    ...
    echo -n "   Cache test: "
    time_start=$(date +%s.%N)
    response=$(curl -s -o /dev/null -w "%{http_code}" "http://VPS_HOST_REDACTED:8003/api/market/overview")
    time_end=$(date +%s.%N)
    ...
    echo ""
    echo "4. API Response:"
    curl -s "http://VPS_HOST_REDACTED:8003/api/market/overview" | python3 -m json.tool | head -30
else
