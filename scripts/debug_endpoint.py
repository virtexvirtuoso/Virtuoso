...
async def test_endpoints():
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get('http://5.223.63.4:8001/api/dashboard-cached/signals') as resp:
        ...
        async with session.get('http://5.223.63.4:8001/api/dashboard-cached/mobile-data') as resp:
            data = await resp.json()
        ...
        async with session.get('http://5.223.63.4:8001/api/fast/signals') as resp:
            data = await resp.json()
