...
scp src/core/exchanges/bybit.py linuxuser@${VPS_HOST}:/home/linuxuser/trading/Virtuoso_ccxt/src/core/exchanges/
...
if ssh linuxuser@${VPS_HOST} "sudo systemctl is-active virtuoso.service" | grep -q "active"; then
    ...
    echo "📋 Service logs:"
    ssh linuxuser@${VPS_HOST} "sudo journalctl -u virtuoso.service --lines=20 --no-pager"
    exit 1
fi
