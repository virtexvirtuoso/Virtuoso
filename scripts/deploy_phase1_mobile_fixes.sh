...
MOBILE_RESPONSE=$(curl -s "http://${VPS_HOST}:8003/api/dashboard/mobile-data" | jq -r '.status // "no_status"')
echo "Mobile endpoint status: $MOBILE_RESPONSE"

# Check for SYSTEM_STATUS contamination
CONFLUENCE_COUNT=$(curl -s "http://${VPS_HOST}:8003/api/dashboard/mobile-data" | jq '.confluence_scores | length')
