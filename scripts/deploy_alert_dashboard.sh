...
curl -s "http://${VPS_HOST}:8000/api/alerts/persisted/stats?hours=24" | python3 -c "
import sys
import json
try:
...
ALERT_COUNT=$(curl -s "http://${VPS_HOST}:8000/api/alerts/persisted/list?hours=24&limit=5" | python3 -c "
import sys
import json
try:
    data = json.load(sys.stdin)
...
echo "  Desktop: http://${VPS_HOST}:8000/"
