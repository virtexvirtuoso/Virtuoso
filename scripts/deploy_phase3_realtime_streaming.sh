...
PHASE3_RESPONSE=$(curl -s "http://${VPS_HOST}:8003/api/phase3/mobile/status" 2>/dev/null || echo '{"error": "endpoint_not_available"}')
...
WS_TEST=$(curl -s -I "http://${VPS_HOST}:8003/ws/mobile" 2>/dev/null | head -1 || echo "HTTP/1.1 404 Not Found")
if [[ "$WS_TEST" == *"404"* ]]; then
...
CHANNELS_RESPONSE=$(curl -s "http://${VPS_HOST}:8003/api/phase3/mobile/channels" 2>/dev/null || echo '{"error": "endpoint_not_available"}')
...
HEALTH_RESPONSE=$(curl -s "http://${VPS_HOST}:8003/health")
...
if [ "$MOBILE_STATUS" = "success" ] && [ "$SYSTEM_HEALTH" = "healthy" ]; then
    ...
    echo "  • WebSocket: ws://${VPS_HOST}:8003/ws/mobile"
    echo "  • Status: http://${VPS_HOST}:8003/api/phase3/mobile/status"
    echo "  • Channels: http://${VPS_HOST}:8003/api/phase3/mobile/channels"
    echo "  • Clients: http://${VPS_HOST}:8003/api/phase3/mobile/clients"
