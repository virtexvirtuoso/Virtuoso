...
curl -s "http://${VPS_HOST}:8001/api/dashboard-cached/mobile-data" | jq '.market_breadth'

echo ""
echo "2. Top Movers Test:"
curl -s "http://${VPS_HOST}:8001/api/dashboard-cached/mobile-data" | jq '{gainers: .top_movers.gainers | length, losers: .top_movers.losers | length}'

echo ""
echo "3. First Confluence Score with Price:"
curl -s "http://${VPS_HOST}:8001/api/dashboard-cached/mobile-data" | jq '.confluence_scores[0] | {symbol, score, price, change_24h}'

echo ""
echo "✅ Deployment Complete!"
echo ""
echo "📱 Test mobile dashboard at: http://${VPS_HOST}:8001/dashboard/mobile"