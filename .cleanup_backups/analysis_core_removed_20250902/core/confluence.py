...
class ConfluenceAnalyzer:
    ...
    def __init__(self, config: Dict[str, Any] = None, interpretation_service=None):
        ...
        # Initialize interpretation generator - use injected service or create new instance
        self.interpretation_generator = interpretation_service or InterpretationGenerator()
    ...
    async def analyze(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            self.logger.info("🚀 CONFLUENCE ANALYZE METHOD CALLED - DEBUG TRACE")
            ...
            for indicator_type, indicator in self.indicators.items():
                try:
                    ...
                    # Log enhanced method usage
                    enhanced_methods_count = 0
                    if hasattr(indicator, '_enhanced_methods_used'):
                        enhanced_methods_count = len(indicator._enhanced_methods_used)
                    elif hasattr(indicator, 'scoring_framework'):
                        enhanced_methods_count = len(indicator.scoring_framework.enhanced_methods)
                    
                    if enhanced_methods_count > 0:
                        self.logger.debug(f"Using {enhanced_methods_count} enhanced methods for {indicator_type}")
            ...
            # Generate interpretations for each component and add them directly to component data
            # Also handle the case where scores dict contains floats instead of full component data
            try:
                self.logger.debug(f"Starting interpretation generation for {len(formatter_results)} components: {list(formatter_results.keys())}")
                for component_name in formatter_results.keys():
                    if component_name == 'top_influential':
                        continue  # Skip the synthetic top_influential component
                        
                    try:
                        self.logger.debug(f"Processing component {component_name} for interpretation")
                        # Try to get interpretation from full component result if available
                        if component_name in results and isinstance(results[component_name], dict):
                            self.logger.debug(f"Using full component result for {component_name}")
                            interpretation = self.interpretation_generator.get_component_interpretation(
                                component_name, results[component_name]
                            )
                        # Otherwise use the score from the scores dict
                        elif component_name in scores:
                            self.logger.debug(f"Using score from scores dict for {component_name}: {scores[component_name]}")
                            interpretation = self.interpretation_generator.get_component_interpretation(
                                component_name, scores[component_name]
                            )
                        else:
                            # Use the score from formatter_results as last resort
                            score_value = formatter_results[component_name].get('score', 50)
                            self.logger.debug(f"Using score from formatter_results for {component_name}: {score_value}")
                            interpretation = self.interpretation_generator.get_component_interpretation(
                                component_name, score_value
                            )
                        
                        if interpretation and component_name in formatter_results:
                            # Add interpretation directly to the component in formatter_results
                            formatter_results[component_name]['interpretation'] = interpretation
                            self.logger.debug(f"Added interpretation for {component_name}: {interpretation[:50]}...")
                    except Exception as e:
                        self.logger.debug(f"Error generating interpretation for {component_name}: {e}")
            except Exception as e:
                self.logger.error(f"Error in interpretation generation loop: {e}")
            
            # PHASE 1: Enhanced Error Detection - Check if interpretations were successfully generated
            self.logger.info("🔍 STARTING INTERPRETATION GENERATION PHASE - DEBUGGING")
            missing_interpretations = []
            for component_name in formatter_results.keys():
                if component_name != 'top_influential':
                    if 'interpretation' not in formatter_results[component_name]:
                        missing_interpretations.append(component_name)
            
            if missing_interpretations:
                self.logger.warning(f"Primary interpretation generation failed for: {missing_interpretations}")
                
                # PHASE 2: Robust Fallback Mechanism - Generate missing interpretations
                for component_name in missing_interpretations:
                    try:
                        interpretation = None
                        
                        # Data source priority order
                        if component_name in results and isinstance(results[component_name], dict):
                            # Use full component data (highest priority)
                            interpretation = self.interpretation_generator.get_component_interpretation(
                                component_name, results[component_name]
                            )
                            self.logger.debug(f"Fallback used full component data for {component_name}")
                        elif component_name in scores:
                            # Use numeric score (medium priority)
                            interpretation = self.interpretation_generator.get_component_interpretation(
                                component_name, scores[component_name]
                            )
                            self.logger.debug(f"Fallback used numeric score for {component_name}: {scores[component_name]}")
                        else:
                            # Use formatter results score (lowest priority)
                            score_value = formatter_results[component_name].get('score', 50)
                            interpretation = self.interpretation_generator.get_component_interpretation(
                                component_name, score_value
                            )
                            self.logger.debug(f"Fallback used formatter score for {component_name}: {score_value}")
                        
                        if interpretation:
                            formatter_results[component_name]['interpretation'] = interpretation
                            self.logger.debug(f"Fallback generated interpretation for {component_name}: {interpretation[:50]}...")
                        else:
                            # Final fallback - generate basic interpretation
                            score = formatter_results[component_name].get('score', 50)
                            interpretation = f"{component_name.replace('_', ' ').title()}: Score {score:.1f}"
                            formatter_results[component_name]['interpretation'] = interpretation
                            self.logger.warning(f"Used basic fallback for {component_name}")
                            
                    except Exception as e:
                        self.logger.error(f"Fallback interpretation failed for {component_name}: {e}")
                        # Ultimate fallback
                        score = formatter_results[component_name].get('score', 50)
                        formatter_results[component_name]['interpretation'] = f"{component_name.replace('_', ' ').title()}: Score {score:.1f}"
            
            # PHASE 3: Guaranteed Data Structure Creation - Create market_interpretations list
            market_interpretations = []
            for component_name in formatter_results.keys():
                if component_name != 'top_influential':
                    interpretation = formatter_results[component_name].get('interpretation')
                    if interpretation:
                        market_interpretations.append({
            ...
            if market_interpretations:
                formatter_results['market_interpretations'] = market_interpretations
                self.logger.info(f"Successfully created market_interpretations with {len(market_interpretations)} components")
            else:
                self.logger.warning("No market_interpretations created - interpretations will not display")
            
            self.logger.debug(f"Finished market interpretations processing, continuing to cross-component insights")
            
            # Generate cross-component insights for enhanced analysis
            try:
                cross_component_insights = self.interpretation_generator.generate_cross_component_insights(results)
                if cross_component_insights:
                    formatter_results['cross_component_insights'] = cross_component_insights
                    self.logger.debug(f"Generated {len(cross_component_insights)} cross-component insights")
            except Exception as e:
                self.logger.debug(f"Error generating cross-component insights: {e}")
            
            # Generate enhanced actionable insights
            try:
                # Get thresholds from config
                thresholds = self.config.get('confluence', {}).get('thresholds', {})
                buy_threshold = thresholds.get('buy', 71)
                sell_threshold = thresholds.get('sell', 35)
                
                actionable_insights = self.interpretation_generator.generate_actionable_insights(
                    results, confluence_score, buy_threshold, sell_threshold
                )
                if actionable_insights:
                    formatter_results['actionable_insights'] = actionable_insights
                    self.logger.debug(f"Generated {len(actionable_insights)} actionable insights")
            except Exception as e:
                self.logger.debug(f"Error generating actionable insights: {e}")
            ...
            self.logger.info("🎯 CONFLUENCE ANALYZE METHOD FINISHING - ABOUT TO RETURN RESPONSE")
    ...
    def _setup_price_structure_attributes(self, price_structure_indicator):
        ...
        if not hasattr(price_structure_indicator, 'timeframe_map'):
            ...
            if self.config and 'timeframes' in self.config:
                try:
                    timeframe_map = {
                        tf: str(self.config.get('timeframes', {}).get(tf, {}).get('interval', 60))
                ...
                if tf in ohlcv_data:
                    ...
                    if isinstance(ohlcv_data[tf], pd.DataFrame):
                        ...
                        if nan_counts.sum() > 0:
                            ...
                            if nan_counts.values.max() < len(ohlcv_data[tf]) * 0.1:
            ...
            for tf_key, tf_data in ohlcv_data.items():
                ...
                if tf_key in standardized:
                    ...
                    tf_map = {
                        '1': 'base',
                        '5': 'ltf',
                        '15': 'ltf',  # 15 minute -> ltf
                        '30': 'mtf',
                        '240': 'htf',
                        '1m': 'base',
                        '5m': 'ltf',
                        '15m': 'ltf',  # 15 minute -> ltf
                        '30m': 'mtf',
                        '4h': 'htf',
