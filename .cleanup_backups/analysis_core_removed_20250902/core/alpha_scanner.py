...
class AlphaScannerEngine:
    ...
    def __init__(self, exchange_manager: ExchangeManager, config: Dict = None):
        ...
        # Minimum volume thresholds (in USDT) - Lowered for broader detection
        self.min_volume_24h = self.config.get('alpha_scanner', {}).get('min_volume_24h', 100000)
    ...
    async def _analyze_symbol_opportunity(self, 
        ...
        try:
            ...
            if not confluence_result or confluence_result.get('score', 0) < 40:
        ...
        try:
            ...
            for timeframe in timeframes:
                ...
                try:
                    ...
                    if not ohlcv or len(ohlcv) < 10:  # Reduced minimum data requirement
                        continue
    ...
    def _assess_momentum_strength(self, confluence_result: Dict) -> float:
        ...
        # Defensive programming: handle None input
        if confluence_result is None:
            return 50.0  # Default neutral score
        
        components = confluence_result.get('components', {})
        
        # Defensive programming: ensure components is a dictionary
        if not isinstance(components, dict):
            return 50.0  # Default neutral score
        
        technical = components.get('technical', {})
        
        # Defensive programming: ensure technical is a dictionary
    ...
    def _calculate_price_levels(self, market_data: Dict, confluence_result: Dict) -> Dict[str, float]:
        ...
        try:
            ...
            atr = talib.ATR(df['high'].values.astype(float), df['low'].values.astype(float), df['close'].values.astype(float), timeperiod=14).iloc[-1]
        ...
        try:
            ...
            for exchange_id, exchange in self.exchange_manager.exchanges.items():
                try:
                    ...
                    usdt_pairs = [market['symbol'] for market in markets if market.get('symbol', '').endswith('USDT') and market.get('active', True)]
