    ...
    def get_component_interpretation(self, component: str, data: Union[Dict[str, Any], float, int]) -> str:
        ...
        try:
            ...
            if not isinstance(data, dict):
                self.logger.warning(f"Component {component} received unexpected data type: {type(data)}")
    ...
    def _interpret_unknown_data(self, component: str, data: Any) -> str:
        ...
        try:
            ...
            if hasattr(data, '__str__'):
                data_str = str(data)
                if len(data_str) < 50:  # Only include if reasonably short
                    return f"{display_name}: {data_str} - Data format not recognized for detailed analysis"
