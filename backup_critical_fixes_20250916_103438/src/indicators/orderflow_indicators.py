...
class OrderflowIndicators(BaseIndicator, DebugLoggingMixin):
    ...
    def __init__(self, config: Dict[str, Any], logger: Optional[Logger] = None):
        ...
        default_weights = {
            'cvd': 0.22,                  # Cumulative Volume Delta (buy vs sell volume) - increased from 0.20
            'trade_flow_score': 0.17,     # Buy vs sell trade flow - increased from 0.15
            'imbalance_score': 0.13,      # Trades-based imbalance (temporal flow analysis) - increased from 0.12
            'open_interest_score': 0.15,   # Open interest analysis - kept same
            'pressure_score': 0.08,       # Trades-based pressure (trade aggression analysis) - kept same
            'liquidity_score': 0.10,      # Liquidity score based on trade frequency and volume - kept same
            'smart_money_flow': 0.15       # Smart Money Concepts - Order Flow - adjusted to 0.15
        }
    ...
    async def calculate(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            start_component = time.time()
            self._log_component_calculation_header("Smart Money Flow", f"Analyzing smart money smart money flow zones")
            
            liquidity_flow_score = self._calculate_smart_money_flow_score(market_data)
            component_scores['smart_money_flow'] = liquidity_flow_score
            
            self._log_calculation_step("Smart Money Flow Results", {
                "flow_score": liquidity_flow_score,
                "interpretation": "Near smart money flow zones" if abs(liquidity_flow_score - 50) > 20 else "Away from zones"
            })
            
            component_times['smart_money_flow'] = self._log_component_timing("Smart Money Flow", start_component)
            ...
            # 7. Log performance metrics (use calculation time, not total method time)
            ...
            try:
                ...
                interpretation_result = interpreter._interpret_orderflow(interpretation_data)
                
                # Ensure interpretation is always a clean string
                if isinstance(interpretation_result, str):
                    interpretation = interpretation_result
                else:
                    # Convert to string and log warning if not already a string
                    self.logger.warning(f"Interpretation generator returned non-string type: {type(interpretation_result)}")
            ...
            result = self.create_result(
                ...
                metadata={
                    ...
                    'calculation_time': calc_time,  # Report calculation time, not total
    ...
    def _calculate_cvd(self, market_data: Dict[str, Any]) -> float:
        """Calculate Cumulative Volume Delta (CVD) from trade data for order flow analysis.
        
        Cumulative Volume Delta (CVD) is a key orderflow indicator that tracks the net difference
        between buying and selling volume over time. It reveals the underlying pressure and
        sentiment in the market by accumulating the volume delta of each trade.
        
        The CVD helps identify:
        ...
        Algorithm:
        1. Process trade data from multiple possible sources
        2. Classify each trade as buy-side or sell-side initiated
        3. Calculate volume delta for each trade: delta = volume × side_multiplier
        4. Accumulate deltas to form cumulative volume delta
        5. Analyze CVD vs price relationship for divergence signals
        6. Normalize and score the CVD relative to price movement
        
        Mathematical Model:
            For each trade i:
            delta_i = volume_i × (1 if buy_side else -1)
            CVD = Σ(delta_i) for all trades in period
            
            CVD_percentage = (CVD / total_volume) × 100
            divergence_score = correlation(price_change, CVD_change)
            
            final_score = base_score + divergence_bonus
        
        Args:
            market_data (Dict[str, Any]): Market data dictionary containing:
                - 'trades' (List|DataFrame): Trade records with 'side', 'size', 'price'
                - 'processed_trades' (List): Pre-processed trade data (preferred)
                - 'ohlcv' (DataFrame): OHLC data for price change calculation
                
        Returns:
            float: CVD score in range 0-100 where:
                - 0-20: Strong net selling pressure (bearish CVD)
                - 20-40: Moderate selling pressure  
                - 40-60: Balanced buying/selling (neutral CVD)
                - 60-80: Moderate buying pressure
                - 80-100: Strong net buying pressure (bullish CVD)
                
        Key Metrics Calculated:
            - Raw CVD: Absolute cumulative volume delta
            - CVD Percentage: CVD as percentage of total volume
            - Price-CVD Divergence: Correlation between price and volume flow
            - CVD Momentum: Rate of change in cumulative delta
            
        Example:
            >>> market_data = {
            ...     'trades': [
            ...         {'side': 'buy', 'size': 1000, 'price': 50000},
            ...         {'side': 'sell', 'size': 800, 'price': 49950},
            ...         {'side': 'buy', 'size': 1200, 'price': 50050}
            ...     ],
            ...     'ohlcv': price_dataframe
            ... }
            >>> cvd_score = indicator._calculate_cvd(market_data)
            >>> print(f"CVD Score: {cvd_score:.2f}")  # Output: CVD Score: 72.35
            
        Interpretation:
            - Positive CVD + Rising Price: Strong bullish confirmation
            - Positive CVD + Falling Price: Bullish divergence (potential reversal)
            - Negative CVD + Rising Price: Bearish divergence (potential reversal)  
            - Negative CVD + Falling Price: Strong bearish confirmation
            
        Note:
            - Uses advanced caching for performance optimization
            - Handles multiple trade data formats automatically
            - Incorporates divergence analysis for enhanced signal quality
            - Critical component of institutional orderflow detection
        """
    ...
    def _calculate_trade_flow_score(self, market_data: Dict[str, Any]) -> float:
        """Calculate trade flow score based on buy vs sell volume directional analysis.
        
        This method analyzes the directional flow of trades by comparing buy-side vs sell-side
        volume to determine market sentiment and momentum. The score indicates the strength
        and direction of order flow pressure.
        
        Algorithm:
        1. Extract trade data from multiple possible sources (processed_trades, trades)
        2. Aggregate buy volume and sell volume separately
        3. Calculate flow ratio: (buy_volume - sell_volume) / total_volume
        4. Apply exponential transformation and normalization
        5. Return score where >50 = bullish flow, <50 = bearish flow
        
        Mathematical Model:
            flow_ratio = (buy_vol - sell_vol) / total_vol
            flow_score = 50 + (flow_ratio * 50)
            final_score = sigmoid_transform(flow_score) * 100
        
        Args:
            market_data (Dict[str, Any]): Market data dictionary containing:
                - 'processed_trades' (List): Processed trade records (preferred)
                - 'trades' (List|DataFrame): Raw trade data (fallback)
                - Each trade should have 'side', 'size', 'price' fields
                
        Returns:
            float: Trade flow score in range 0-100 where:
                - 0-30: Strong bearish flow (high sell pressure)
                - 30-45: Moderate bearish flow
                - 45-55: Neutral/balanced flow
                - 55-70: Moderate bullish flow  
                - 70-100: Strong bullish flow (high buy pressure)
                
        Raises:
            DataUnavailableError: When no valid trade data is found
            ValueError: When trade data format is invalid
            
        Example:
            >>> market_data = {
            ...     'trades': [
            ...         {'side': 'buy', 'size': 100, 'price': 50000},
            ...         {'side': 'sell', 'size': 80, 'price': 49950}
            ...     ]
            ... }
            >>> score = indicator._calculate_trade_flow_score(market_data)
            >>> print(f"Flow score: {score:.2f}")  # Output: Flow score: 65.22
            
        Note:
            - Uses caching to avoid redundant calculations within same analysis cycle
            - Handles multiple data formats for maximum compatibility
            - Returns cached value if available to improve performance
        """
        # Use caching to avoid redundant calculations
        ...
        try:
                ...
                try:
                    ...
                    if buy_volume == 0 and sell_volume == 0:
                        ...
                        if not candle_trades.empty:
                            ...
                            # Use vectorized string formatting instead of iterrows
                            trade_info = []
                            for i, trade in enumerate(sample_trades.to_dict('records')):
                                trade_info.append(f"Trade {i}: side={trade['side']}, amount={trade['amount']}, is_buy={trade.get('is_buy', False)}, is_sell={trade.get('is_sell', False)}")
                            self.logger.warning("\n".join(trade_info))
            ...
            if getattr(self, 'interpolate_missing_cvd', True) and 0 in candle_cvds:
                ...
                if zero_count > 0:
                    ...
                    # Optimize: Calculate max_abs_cvd once instead of for each zero index (O(n) instead of O(n²))
                    non_zero_cvds = [abs(cvd) for cvd in candle_cvds if cvd != 0]
                    max_abs_cvd = max(non_zero_cvds) if non_zero_cvds else 1.0
                    fallback_value = max_abs_cvd * 0.01  # 1% of max as fallback
                    
                    for idx in zero_indices:
                        candle_cvds[idx] = fallback_value
    ...
    # ===== SMART MONEY CONCEPTS - FLOW ANALYSIS =====
    # Analyzes institutional order flow patterns and accumulation/distribution zones
    
    def _calculate_smart_money_flow_score(self, market_data: Dict[str, Any]) -> float:
        """Calculate smart money flow zones score across multiple timeframes for institutional activity detection.
        
        This method identifies liquidity zones and smart money flow patterns by analyzing price action
        around key levels where institutional traders typically accumulate or distribute positions.
        The analysis spans multiple timeframes to capture both immediate and longer-term institutional activity.
        
        Smart Money Concepts Analyzed:
        1. **Liquidity Sweeps**: Price movements that target stops and liquidity pools
        2. **Order Blocks**: Significant support/resistance levels from institutional activity  
        3. **Fair Value Gaps**: Price imbalances indicating rapid institutional movement
        4. **Market Structure**: Higher highs/lows and structural breaks
        5. **Volume Profile**: Price levels with high institutional activity
        
        Algorithm:
        1. Extract multi-timeframe OHLCV data (HTF, MTF, LTF, base)
        2. For each timeframe, detect smart money flow patterns:
           - Identify swing highs/lows using configurable swing length
           - Cluster price levels within percentage range
           - Create liquidity zones around significant levels
           - Check for liquidity sweeps and order block formation
        3. Calculate proximity scores based on current price vs smart money zones
        4. Weight scores by timeframe importance (HTF=40%, MTF=35%, LTF=25%)
        5. Return composite score indicating smart money activity strength
        
        Mathematical Model:
            For each timeframe t:
            swing_points = detect_swings(ohlcv_t, swing_length)
            clusters = cluster_points(swing_points, range_percent)  
            zones = create_liquidity_zones(clusters)
            proximity_t = calculate_proximity(current_price, zones)
            
            smart_money_score = Σ(weight_t × proximity_t) for all timeframes
        
        Args:
            market_data (Dict[str, Any]): Market data containing multi-timeframe OHLCV data:
                - 'ohlcv' (Dict): Dictionary with timeframe keys ('1h', '4h', '1d', etc.)
                - Each timeframe should contain DataFrame with OHLC columns
                - Minimum 50 periods per timeframe recommended for accuracy
                
        Returns:
            float: Smart money flow score in range 0-100 where:
                - 0-20: No significant smart money activity detected
                - 20-40: Weak smart money presence
                - 40-60: Moderate smart money activity
                - 60-80: Strong smart money presence (bullish/bearish)
                - 80-100: Very strong institutional activity detected
                
        Configuration:
            - swing_length: Number of periods for swing detection (default: 25)
            - range_percent: Clustering range for price levels (default: 0.01 = 1%)
            - timeframe_weights: Importance of each timeframe analysis
            
        Example:
            >>> market_data = {
            ...     'ohlcv': {
            ...         '1h': hourly_ohlcv_df,
            ...         '4h': four_hour_ohlcv_df,
            ...         '1d': daily_ohlcv_df
            ...     }
            ... }
            >>> score = indicator._calculate_smart_money_flow_score(market_data)
            >>> print(f"Smart Money Score: {score:.2f}")  # Output: Smart Money Score: 73.45
            
        Note:
            - Higher timeframes (daily, 4h) carry more weight in final score
            - Requires sufficient historical data for reliable pattern detection
            - Score updates as price approaches or moves away from smart money zones
            - Combines with other orderflow metrics for comprehensive analysis
        """
        try:
            # Extract OHLCV data from market_data
            ohlcv_data = market_data.get('ohlcv', {})
            if not ohlcv_data:
                self.logger.warning("No OHLCV data available for smart money flow zones analysis")
                return 50.0
            ...
            for tf, weight in weights.items():
                ...
                # Detect smart money flow zones
                smart_money_flow = self._detect_smart_money_flow(df)
                
                # Calculate score based on proximity and sweeps
                current_price = df['close'].iloc[-1]
                tf_score = self._score_smart_money_proximity(current_price, smart_money_flow)
                
                scores.append(tf_score * weight)
                self.logger.debug(f"Smart money flow zones score for {tf}: {tf_score:.2f}")
            
            final_score = float(np.sum(scores)) if scores else 50.0
            self.logger.debug(f"Final smart money flow zones score: {final_score:.2f}")
            return final_score
            
        except Exception as e:
            self.logger.error(f"Error calculating smart money flow zones score: {str(e)}")
            return 50.0

    def _detect_smart_money_flow(self, df: pd.DataFrame, swing_length: int = 25, range_percent: float = 0.01) -> Dict[str, Any]:
        """
        Detect smart money flow zones where multiple swing highs/lows cluster.
        ...
        Args:
            df: OHLCV DataFrame
            swing_length: Length for swing detection (optimized to 25)
            range_percent: Percentage range for clustering
            
        Returns:
            Dict containing smart money flow zone information
        """
        try:
            # Performance optimization: Limit analysis to last 500 candles
            if len(df) > 500:
                df = df.iloc[-500:].copy()
            ...
            # Group nearby swings into smart money flow zones
            smart_money_flow = {
            ...
            self.logger.error(f"Error detecting smart money flow zones: {str(e)}")
            return {'bullish': [], 'bearish': []}

    def _detect_swing_highs_lows(self, df: pd.DataFrame, swing_length: int) -> List[Tuple[int, float, int]]:
        """Detect swing highs and lows for smart money flow analysis."""
        swings = []
    ...
    def _create_liquidity_zone(self, df: pd.DataFrame, cluster: List[Tuple[int, float]], zone_type: str) -> Dict[str, Any]:
        """Create a smart money flow zone from a cluster of swing points."""
    ...
    def _check_liquidity_sweeps(self, df: pd.DataFrame, smart_money_flow: Dict[str, List]) -> None:
        """
        Check if smart money flow zones have been swept by smart money.
        ...
        try:
            ...
            for zone in smart_money_flow['bullish']:
                if zone['swept']:
                    continue
            ...
            for zone in smart_money_flow['bearish']:
                if zone['swept']:
                    continue
            ...
            self.logger.error(f"Error checking smart money sweeps: {str(e)}")

    def _score_smart_money_proximity(self, current_price: float, smart_money_flow: Dict[str, List]) -> float:
        """
        Score based on proximity to smart money flow zones and sweep patterns.
    ...
    def _log_component_specific_alerts(self, component_scores: Dict[str, float], 
        ...
        flow_score = component_scores.get('smart_money_flow', 50)
        if flow_score >= 75:
            alerts.append("Smart Money Flow Bullish - Near smart money support zones")
        elif flow_score <= 25:
            alerts.append("Smart Money Flow Bearish - Near smart money resistance zones")