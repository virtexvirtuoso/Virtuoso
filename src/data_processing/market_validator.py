...
class MarketDataValidator:
    ...
    async def validate_market_message(
        self,
        data: Dict[str, Any],
        data_type: str,
        strict: bool = False
        ...
        try:
            ...
            if data_type == 'kline':
                ...
                # Use time series validator for funding data
                result = await self.validate_ticker(data, strict)
            elif data_type == 'open_interest':
                # Use time series validator for open interest data
                result = await self.validate_ticker(data, strict)
            else:
    ...
    async def validate_kline(
        ...
        # Extract data from kline list format [timestamp, open, high, low, close, volume, turnover]
        ...
        candle = kline['list'][0]  # Get most recent candle
        
        # Convert timestamp to datetime
        timestamp = int(candle[0])  # startTime
        start_time = datetime.fromtimestamp(timestamp / 1000) if timestamp > 1e12 else datetime.fromtimestamp(timestamp)
        
        validation_data = {
            'symbol': kline.get('symbol', ''),
            'price': float(candle[4]),  # closePrice
            'volume': float(candle[5]),  # volume
            'start_time': start_time,
            'end_time': start_time + timedelta(minutes=1)  # 1min default interval
        }
        
        # Additional kline-specific validation
