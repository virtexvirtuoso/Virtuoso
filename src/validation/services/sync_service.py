...
Merged from: core/validation/service.py and validation/services/sync_service.py
"""

from ..core.base import ValidationContext, ValidationResult, ValidationProvider, ValidationRegistry
from ..core.exceptions import ValidationError
from ..core.models import ValidationRule, ValidationLevel, ValidationErrorData
from ..core.protocols import ValidatorProtocol
...
class AsyncValidationService(ValidationProvider):
    ...
    def __init__(self, registry: Optional[ValidationRegistry]=None, max_errors: int=100, strict_mode: bool=False):
    ...
    async def validate(self, data: Any, context: ValidationContext) -> ValidationResult:
        ...
        if validator:
            try:
                ...
                result.add_error(ValidationError(code='validator_error', message=f'Validator error: {str(e)}', level=ValidationLevel.ERROR))
        ...
        for rule in rules:
            ...
            try:
                ...
                result.add_error(ValidationError(code='rule_error', message=f'Rule error: {str(e)}', level=ValidationLevel.ERROR))
                if context.strict_mode or self._strict_mode:
                    break
        return result
    ...
    async def validate_batch(self, data: List[Dict]) -> List[Dict]:
        ...
        return [item for item in data if isinstance(item, dict) and 'turnover24h' in item and ('symbol' in item)]
    ...
    async def _validate_item(self, item: Dict) -> bool:
        ...
        if 'turnover24h' in item:
            try:
                ...
                return False
        return all(await asyncio.gather(*[rule.check(item) for rule in self._rules.get(item['symbol'], [])]))
