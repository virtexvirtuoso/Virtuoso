...
async def get_mobile_dashboard_data() -> Dict[str, Any]:
    ...
    Consolidated from mobile_direct.py for better maintenance
    """
    try:
        if USE_DIRECT_CACHE and direct_cache:
            # Use optimized cache adapter
            data = await direct_cache.get_mobile_data()
            return data
        else:
            # Fallback to service-based data
            service = get_dashboard_integration()
            confluence_scores = []
            
            # Get data from service if available
            if hasattr(service, 'latest_signals') and service.latest_signals:
                for symbol, data in list(service.latest_signals.items())[:10]:
                    confluence_scores.append({
                        "symbol": symbol,
            ...
            # Default data if no service data available
