...
class MarketMonitor:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Initialize whale activity monitoring with higher thresholds to reduce noise
        self.whale_activity_config = self.config.get('monitoring', {}).get('whale_activity', {
            'enabled': True,
            'accumulation_threshold': 5000000,  # ↑ Increased from 1M to 5M USD threshold
            'distribution_threshold': 5000000,  # ↑ Increased from 1M to 5M USD threshold
            ...
            'min_order_count': 8,               # ↑ Increased from 5 to 8 whale orders
            'market_percentage': 0.05,          # ↑ Increased from 0.02 to 0.05 (5% of market volume)
            'excluded_symbols': [],             # No excluded symbols
            'included_symbols': []              # Monitor all symbols
        })
        ...
        try:
            ...
            self.logger.error(f"Failed to initialize manipulation detector: {str(e)}")
    ...
    async def _process_symbol(self, symbol: str) -> None:
        ...
        try:
            ...
            # Monitor for manipulation activity
            self.logger.info(f"Monitoring manipulation activity for {symbol_str}")
            await self._monitor_manipulation_activity(symbol_str, market_data)
    ...
    async def _monitor_manipulation_activity(self, symbol: str, market_data: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            if not hasattr(self, 'manipulation_detector') or not self.manipulation_detector:
    ...
    async def _store_manipulation_data(self, symbol: str, manipulation_alert) -> None:
        ...
        """
        try:
            if not hasattr(self, 'database_client') or not self.database_client:
                return
