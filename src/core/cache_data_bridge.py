    ...
    async def _bridge_signals_data(self):
        ...
        try:
            ...
            if not signals_data or len(signals_data.get('signals', [])) == 0:
                ...
                if not tickers:
                    logger.warning("Could not fetch real market data - using direct exchange API fallback")
                    # Instead of creating fake SYSTEM_STATUS, try direct exchange fallback
                    fallback_tickers = await self._get_direct_exchange_fallback()
                    if fallback_tickers:
                        tickers = fallback_tickers
                        logger.info(f"Using direct exchange fallback data: {len(tickers)} symbols")
                    else:
                        logger.error("All data sources failed - storing empty signals instead of fake data")
                        # Store empty signals instead of fake SYSTEM_STATUS data
                        signals_data = {
                            'signals': [],
                            'count': 0,
                            'timestamp': int(time.time()),
