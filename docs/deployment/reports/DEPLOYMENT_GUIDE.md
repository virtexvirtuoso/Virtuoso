...
```bash
# Application logs
tail -f /opt/virtuoso/logs/virtuoso.log

# Docker logs
docker logs -f virtuoso --tail=100
...
```bash
# Check logs
sudo journalctl -u virtuoso -n 100
...
cp /opt/virtuoso/.env $BACKUP_DIR/
cp -r /opt/virtuoso/config $BACKUP_DIR/

# Backup data
cp -r /opt/virtuoso/data $BACKUP_DIR/
cp -r /opt/virtuoso/reports $BACKUP_DIR/
...
journalctl -u virtuoso -f  # System logs
