...
def main():
    ...
    if not run_command(f'ssh linuxuser@VPS_HOST_REDACTED "{vps_command}"', "Executing VPS deployment"):
        print("❌ VPS deployment failed")
        return 1
    
    # Verify deployment
    print("🔍 Verifying deployment...")
    log_command = 'ssh linuxuser@VPS_HOST_REDACTED "sudo journalctl -u virtuoso.service --since \\"2 minutes ago\\" -n 30 --no-pager"'
    ...
    if run_command(log_command, "Checking service logs", check=False):
        ...
        print("   curl http://VPS_HOST_REDACTED:8003/api/dashboard/mobile-data")
        ...
        print("   ssh linuxuser@VPS_HOST_REDACTED")
