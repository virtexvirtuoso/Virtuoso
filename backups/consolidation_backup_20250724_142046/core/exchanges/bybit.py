...
class BybitExchange(BaseExchange):
    ...
    def __init__(self, config: Dict[str, Any], error_handler: Optional[Any] = None):
        ...
        # Initialize HTTP session placeholder - will be created in initialize()
    ...
    async def _init_rest_client(self) -> bool:
        ...
        try:
            ...
            if self._http_session is None:
                ...
                self.logger.info("Created shared HTTP session with connection pooling")
    ...
    async def _test_rest_connection(self) -> bool:
        ...
        try:
            ...
            return False
    async def _make_request_with_retry(self, method: str, endpoint: str, params: Optional[Dict[str, Any]] = None, max_retries: int = 2) -> Dict[str, Any]:
        """Make a request with retry logic for critical endpoints."""
        for attempt in range(max_retries + 1):
            try:
                result = await self._make_request(method, endpoint, params)
                
                # If we get a successful response (retCode 0) or a non-network error, return it
                ...
                self.logger.debug(f"Retrying {endpoint} in {wait_time}s (attempt {attempt + 1}/{max_retries + 1})")
                await asyncio.sleep(wait_time)
                
            except Exception as e:
                if attempt == max_retries:
                    self.logger.error(f"Request to {endpoint} failed after {max_retries + 1} attempts: {str(e)}")
    ...
    async def _make_request(self, method: str, endpoint: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._http_session and not self._http_session.closed:
                ...
                close_session = True
                self.logger.debug("Using temporary session (shared session not available)")
            ...
            self.logger.warning(f"HTTP client error for {endpoint}: {str(e)}")
            return {'retCode': -1, 'retMsg': f'HTTP client error: {str(e)}'}
    ...
    async def close(self) -> None:
        ...
        try:
            ...
            if self._http_session and not self._http_session.closed:
                ...
                self._http_session = None
                self.logger.info("Closed shared HTTP session")
                
            # Close legacy session if exists
            if hasattr(self, 'session') and self.session:
                await self.session.close()
    ...
    async def _fetch_ohlcv(self, symbol: str, interval: str, limit: int = 1000) -> List[List[Any]]:
        ...
        try:
            ...
            request_limit = min(limit, 200)  # Bybit API limit
            ...
            minutes_back = timeframe_minutes * request_limit * 1.2
            ...
            response = await self._make_request('GET', '/v5/market/kline', {
                ...
                'limit': request_limit,  # Use calculated limit
                'start': start_time,
                'end': end_time
            })
    ...
    async def fetch_ohlcv(self, symbol: Union[str, dict], timeframe: str = '1m', limit: int = 1000, since: Optional[int] = None) -> List[List[float]]:
        ...
        try:
            ...
            # Make the API request with retry logic for timeouts
    ...
    async def fetch_trades(self, symbol: Union[str, dict], since: Optional[int] = None, limit: Optional[int] = None, params=None) -> List[Dict[str, Any]]:
    ...
    async def _fetch_ticker(self, symbol: str) -> dict:
        ...
        try:
            ...
            # Make the API request with retry for better resilience
            response = await self._make_request_with_retry('GET', '/v5/market/tickers', {
                'category': category,
                'symbol': api_symbol
            })
    ...
    async def _calculate_historical_volatility(self, symbol: str, timeframe: str = '5min', window: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            candles = await self._fetch_ohlcv(symbol, timeframe)
            ...
            vol_ema = pd.Series(talib.EMA(volatility.values.astype(float), timeperiod=5), index=volatility.index)
            if len(vol_ema) >= 2:
