...
class HealthMonitor:
    ...
    def _check_threshold_violations(self) -> None:
        """Check for threshold violations and generate alerts."""
        # Enhanced debugging: Log entry with current system state
        self.logger.debug(f"=== MEMORY THRESHOLD CHECK DEBUG ===")
        ...
        self.logger.debug(f"Current memory usage: {current_memory_percent:.1f}% ({current_memory_mb:.0f}MB)")
        self.logger.debug(f"Total memory: {total_memory_mb:.0f}MB")
        self.logger.debug(f"Available memory: {available_memory_mb:.0f}MB")
        ...
        self.metrics['cpu'].thresholds['critical'] = cpu_threshold + 10
        ...
        # Check Memory - Apply enhanced memory monitoring with intelligent analysis
        memory_tracking_config = self.config.get('monitoring', {}).get('memory_tracking', {})
        enable_enhanced_monitoring = memory_tracking_config.get('enable_enhanced_monitoring', True)
        disable_memory_warnings = memory_tracking_config.get('disable_memory_warnings', False)
        
        # Enhanced debugging: Log memory configuration
        self.logger.debug(f"Enhanced memory monitoring enabled: {enable_enhanced_monitoring}")
        self.logger.debug(f"Memory warnings disabled: {disable_memory_warnings}")
        
        if enable_enhanced_monitoring and not disable_memory_warnings:
            # Use enhanced memory analysis
            self.logger.debug("=== ENHANCED MEMORY ANALYSIS ===")
            memory_analysis = self._get_enhanced_memory_analysis()
            
            # Enhanced debugging: Log analysis results
            self.logger.debug(f"Current memory usage: {memory_analysis['current_usage']:.1f}%")
            self.logger.debug(f"Adaptive warning threshold: {memory_analysis['adaptive_warning_threshold']:.1f}%")
            self.logger.debug(f"Adaptive critical threshold: {memory_analysis['adaptive_critical_threshold']:.1f}%")
            self.logger.debug(f"Trend rate: {memory_analysis['trend_rate']:.1f}%/min")
            self.logger.debug(f"Volatility: {memory_analysis['volatility']:.1f}%")
            self.logger.debug(f"Leak detected: {memory_analysis['leak_detected']}")
            self.logger.debug(f"Leak confidence: {memory_analysis['leak_confidence']:.1%}")
            self.logger.debug(f"Confidence score: {memory_analysis['confidence_score']:.1%}")
            
            # Determine if alert should be triggered
            should_alert, alert_level, confidence_score = self._should_trigger_memory_alert(memory_analysis)
            
            if should_alert:
                self.logger.debug(f"ENHANCED MEMORY ALERT TRIGGERED: {alert_level.upper()} (Confidence: {confidence_score:.1%})")
                self._create_enhanced_memory_alert(memory_analysis, alert_level, confidence_score)
            else:
                self.logger.debug(f"No enhanced memory alert triggered - conditions not met")
                
            self.logger.debug("=== ENHANCED MEMORY ANALYSIS COMPLETE ===")
        else:
            # Fall back to original memory monitoring logic
            self.logger.debug("Using original memory monitoring logic")
            
            memory_warning_threshold = memory_tracking_config.get('warning_threshold_percent', 
                                                                 self.config.get('memory_warning_threshold', 98))
            memory_critical_threshold = memory_tracking_config.get('critical_threshold_percent', 
                                                                  self.config.get('memory_critical_threshold', 98))
            min_warning_size_mb = memory_tracking_config.get('min_warning_size_mb', 2048)
            ...
            if current_memory_mb >= min_warning_size_mb:
                if self.metrics['memory'].is_critical():
                    self.logger.debug(f"CRITICAL MEMORY ALERT TRIGGERED: {current_memory_percent:.1f}% >= {memory_critical_threshold}%")
                ...
                elif self.metrics['memory'].is_warning():
                    should_suppress = suppress_repeated and self._has_recent_memory_warning()
                    
                    if not should_suppress:
                        self.logger.debug(f"WARNING MEMORY ALERT TRIGGERED: {current_memory_percent:.1f}% >= {memory_warning_threshold}%")
                    ...
                    self.logger.debug(f"No memory alert triggered - usage {current_memory_percent:.1f}% below thresholds")
            else:
                self.logger.debug(f"Memory usage below minimum size threshold ({min_warning_size_mb}MB) - skipping alert checks")
        ...
        self.logger.debug(f"=== MEMORY THRESHOLD CHECK DEBUG COMPLETE ===")
    ...
    def _get_enhanced_memory_analysis(self) -> Dict[str, Any]:
        """Get enhanced memory analysis with trend detection and adaptive thresholds."""
        try:
            memory = psutil.virtual_memory()
            current_time = time.time()
            
            # Get memory history for trend analysis
            memory_values = self.metrics['memory'].values
            memory_timestamps = self.metrics['memory'].timestamps
            
            # Calculate trend over last 10 measurements
            trend_window = min(10, len(memory_values))
            if trend_window >= 3:
                recent_values = memory_values[-trend_window:]
                recent_times = memory_timestamps[-trend_window:]
                ...
                if len(recent_times) > 1:
                    ...
                    if avg_time_diff > 0:
                        value_diffs = [recent_values[i] - recent_values[i-1] for i in range(1, len(recent_values))]
                        avg_value_diff = sum(value_diffs) / len(value_diffs)
                        trend_rate = (avg_value_diff / avg_time_diff) * 60  # per minute
                    else:
                        trend_rate = 0
                else:
                    trend_rate = 0
            ...
            if len(memory_values) >= 20:
                ...
                first_quarter = memory_values[:5]
                last_quarter = memory_values[-5:]
                if (np.mean(last_quarter) > np.mean(first_quarter) + 5 and  # 5% increase
                    all(last_quarter[i] >= last_quarter[i-1] for i in range(1, len(last_quarter)))):
                    leak_detected = True
                    leak_confidence = min(1.0, (np.mean(last_quarter) - np.mean(first_quarter)) / 10)
            ...
            base_warning = self.config.get('memory_warning_threshold', 98)
            base_critical = self.config.get('memory_critical_threshold', 98)
            
            # Adjust thresholds based on CPU load
            cpu_adjustment = max(0, (cpu_percent - 50) / 100)  # Reduce threshold if CPU is high
            load_adjustment = max(0, (load_avg[0] - 2) / 10) if load_avg[0] > 2 else 0
            
            adaptive_warning = base_warning - (cpu_adjustment + load_adjustment) * 5
            adaptive_critical = base_critical - (cpu_adjustment + load_adjustment) * 5
            
            # Calculate confidence score for alert
            confidence_score = 0.0
            current_usage = memory.percent
            
            # Base confidence on how far above threshold
            if current_usage >= adaptive_critical:
                confidence_score = min(1.0, (current_usage - adaptive_critical) / 10 + 0.8)
            elif current_usage >= adaptive_warning:
                confidence_score = min(1.0, (current_usage - adaptive_warning) / 10 + 0.5)
            
            # Adjust confidence based on trend
            if trend_rate > 2:  # Rapidly increasing
                confidence_score = min(1.0, confidence_score + 0.2)
            elif trend_rate < -2:  # Rapidly decreasing
            ...
            process_memory = self._get_process_memory_details()
            ...
            return {
                ...
                'trend_rate': trend_rate,  # percentage per minute
                'volatility': volatility,
                'leak_detected': leak_detected,
                'leak_confidence': leak_confidence,
                'cpu_percent': cpu_percent,
                'load_average': load_avg,
                'adaptive_warning_threshold': adaptive_warning,
                'adaptive_critical_threshold': adaptive_critical,
            ...
            self.logger.error(f"Error in enhanced memory analysis: {str(e)}")
            return {
                'current_usage': 0,
                'available_mb': 0,
                'total_mb': 0,
                ...
                'cpu_percent': 0,
                'load_average': (0, 0, 0),
                'adaptive_warning_threshold': self.config.get('memory_warning_threshold', 98),
                'adaptive_critical_threshold': self.config.get('memory_critical_threshold', 98),
                'confidence_score': 0,
                'process_memory': [],
                'timestamp': time.time()
            }

    def _get_process_memory_details(self) -> List[Dict[str, Any]]:
        """Get detailed memory information for top processes."""
        try:
            processes = []
            ...
            # Sort by memory usage (descending) and return top 10
            processes.sort(key=lambda x: x['memory_mb'], reverse=True)
            return processes[:10]
            
        except Exception as e:
            self.logger.error(f"Error getting process memory details: {str(e)}")
            return []

    def _should_trigger_memory_alert(self, analysis: Dict[str, Any]) -> Tuple[bool, str, float]:
        """Determine if a memory alert should be triggered based on enhanced analysis.
        
        Returns:
            Tuple[bool, str, float]: (should_alert, alert_level, confidence_score)
        """
        current_usage = analysis['current_usage']
        adaptive_warning = analysis['adaptive_warning_threshold']
        adaptive_critical = analysis['adaptive_critical_threshold']
        ...
        if current_usage >= adaptive_critical:
            return True, "critical", confidence_score
        
        # Check for warning conditions with enhanced logic
        if current_usage >= adaptive_warning:
            # Additional checks for warning alerts
            should_alert = True
            ...
            # Suppress if trend is decreasing rapidly
            if trend_rate < -5:
                should_alert = False
            
            # Enhance alert if memory leak is detected
            if leak_detected and leak_confidence > 0.5:
                confidence_score = min(1.0, confidence_score + leak_confidence * 0.3)
            
            if should_alert:
                return True, "warning", confidence_score
        
        return False, "", 0.0

    def _create_enhanced_memory_alert(self, analysis: Dict[str, Any], alert_level: str, confidence_score: float) -> None:
        """Create an enhanced memory alert with detailed analysis."""
        current_usage = analysis['current_usage']
        available_mb = analysis['available_mb']
        total_mb = analysis['total_mb']
        used_mb = analysis['used_mb']
        trend_rate = analysis['trend_rate']
        volatility = analysis['volatility']
        leak_detected = analysis['leak_detected']
        leak_confidence = analysis['leak_confidence']
        cpu_percent = analysis['cpu_percent']
        process_memory = analysis['process_memory']
        ...
        message = f"{emoji} {status} MEMORY USAGE {emoji}\n"
        message += f"Memory Usage: {current_usage:.1f}% ({used_mb:.0f}MB)\n"
        message += f"Total Memory: {total_mb:.0f}MB\n"
        message += f"Available Memory: {available_mb:.0f}MB\n"
        message += f"System Status: {status}\n"
        
        # Add trend information
        if abs(trend_rate) > 0.5:
            trend_direction = "↗️ Increasing" if trend_rate > 0 else "↘️ Decreasing"
            message += f"Trend: {trend_direction} ({abs(trend_rate):.1f}%/min)\n"
        
        # Add volatility information
        if volatility > 2:
            message += f"Volatility: High ({volatility:.1f}%)\n"
        
        # Add leak detection
        if leak_detected:
            message += f"⚠️ Potential Memory Leak Detected (Confidence: {leak_confidence:.1%})\n"
        
        # Add system load information
        message += f"CPU Usage: {cpu_percent:.1f}%\n"
        
        # Add confidence score
        message += f"Alert Confidence: {confidence_score:.1%}\n"
        
        # Add top processes
        if process_memory:
            message += "\nTop Memory-Consuming Processes:\n"
            for i, proc in enumerate(process_memory[:5]):
                message += f"{i+1}. {proc['name']} (PID: {proc['pid']}): {proc['memory_mb']:.1f}MB ({proc['memory_percent']:.1f}%)\n"
